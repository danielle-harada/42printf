...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/16 22:36:50 by dteixeir          #+#    #+#             */
/*   Updated: 2021/04/17 12:58:21 by dteixeir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void		print_flags(t_flags *flags, va_list args)
{
	if (flags->type == '%')
		print_percent(flags);
...
static void		get_specs(const char *str, t_flags *flags, va_list args)
{
	if (str[flags->count] == '%')
		flags->type = '%';
	else if (str[flags->count] == 'c')
		flags->type = 'c';
...
static void		get_flags_a(const char *str, t_flags *flags, va_list args)
{
	if (str[flags->count] == '.')
	{
		flags->dot = 1;
		flags->count++;
		if (str[flags->count] == '*')
			get_star(flags, args, &flags->precision);
...
void			get_flags(const char *str, t_flags *flags, va_list args)
{
	...
	if (str[flags->count] == '*')
		get_star(flags, args, &flags->width);
	else if (is_number(str, flags) == 1)
